...
def mock_session(mocker) -> MagicMock:
    ...
    mocker.patch("app.data_layer.database.sqlite.crud.websocket_crud.get_session", return_value=iter([mock_session]))
...
# Test: 1
def test_insert_data_single_dict(mock_session: MagicMock, sample_stock_price_info: Dict[str, str]) -> None:
    """
...
# Test: 2
def test_insert_data_single_dict_upsert(mock_session: MagicMock, sample_stock_price_info: Dict[str, str]) -> None:
    """
...
def test_insert_data_empty_input(mock_session: MagicMock, mocker) -> None:
    ...
    """
    mock_logger = mocker.patch("app.data_layer.database.sqlite.crud.websocket_crud.logger")
...
# Test: 4
def test_insert_data_list_dicts(mock_session: MagicMock, sample_stock_price_info: Dict[str, str]) -> None:
    """
...
# Test: 5
def test_insert_data_list_dicts_upsert(mock_session: MagicMock, sample_stock_price_info: Dict[str, str]) -> None:
    """
...
# Test: 7
def test_upsert(mock_session: MagicMock, sample_stock_price_info: Dict[str, str]) -> None:
    """
    Test upsert logic
    """
...
# Test: 8
def test_insert_or_ignore(mock_session: MagicMock, sample_stock_price_info: Dict[str, str]) -> None:
    """
